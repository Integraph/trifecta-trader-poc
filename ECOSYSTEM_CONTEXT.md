...
| Stock Trader Engine | `trifecta-trader-poc` | Python, FastAPI admin, Alpaca | v1.0.2, Tasks 001-020 complete (vendor zero-mod broken — TRI-66). **Engine-first MVP: TRI-66 upgrade is the active priority; live trading (TRI-31) is DEFERRED under the paper-only MVP.** | FastAPI admin API + React admin-ui/ |
...
- Stock Trader Engine: `vendor/TradingAgents` (`TauricResearch/TradingAgents`) — zero modifications is the **goal but currently VIOLATED**: the pin `5de91bc` is a local commit (Task 020) that modifies 4 vendor files. *(Currency: pinned ~v0.2.0; upstream v0.3.0. **TRI-66** upgrades to v0.3.0 and restores zero-mod.)*
