...
**Codex QA round 2 (CHANGES-REQUESTED, 2026-07-01) — accepted and fixed.** Round 2 verified the round-1 repro set fixed but found two remaining override vectors, both confirmed reproducible: **(a)** the decoration class allowed `>`, so a **blockquoted** stale decision (`> Recommendation: Sell`) after the real header won last-match; **(b)** **fenced code blocks** weren't ignored, so a stale decision inside ```` ``` ```` fences also won. Both matter because v0.3.0 injects *prior decisions* into the PM prompt as `past_context` — the PM plausibly quotes an old decision. **Fix:** `>` removed from the decoration class (a blockquote is a quotation, never THE decision), and fenced code blocks are stripped before method-0 matching; an **unterminated** fence strips to end-of-text (losing a real decision to a loud UNKNOWN beats silently adopting a stale one). +5 adversarial tests (blockquote after/before, fenced after, fenced-only, unterminated fence) — **42/42 signal tests pass**, and all five preserved real outputs (cloud + local, all runs) extract identically, so the §9 smoke evidence remains valid without re-running the pipeline (the change is extraction-only; re-runs would clobber evidence per TRI-79).

**Documented residual ceiling (unchanged in kind, narrowed in surface):** a *plain* prose line that itself *begins* with `<Label>: <rating>` (not blockquoted, not fenced, qualifier from the allowlist or none) still matches — regex scraping cannot fully disambiguate that, and the local model **changes its label format run to run**, so the allowlist can only chase. The durable fix is structured `PortfolioDecision` extraction, owned by **TRI-70**. (Methods 1–3, the pre-TRI-66 legacy patterns, retain their historical unanchored behavior — unchanged by this task and likewise superseded by TRI-70.)
