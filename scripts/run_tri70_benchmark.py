...
def _aggregate(config_name: str, runs: list) -> dict:
    ...
    consensus = consensus_with_abstention(ok) if ok else None
...
def _print_summary(summary):
    ...
    hdr = (
        f"{'Config@Ticker':<32}{'Modal':>7}{'Agree':>7}{'Consensus':>11}"
        f"{'Extract':>8}{'Q mean':>8}{'Q σ':>6}{'Wall s':>8}"
    )
    print(hdr)
    print("-" * 90)
    for s in summary:
        ...
        consensus_decision = consensus.get("decision", "N/A")
        print(f"{s['config']:<32}{str(s['modal_decision']):>7}{s['decision_agreement']:>7}"
              f"{consensus_decision:>11}"
