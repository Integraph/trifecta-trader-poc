...
async def get_health():
    ...
    Only `status` and `color` are read by the badge; subsystems are for the admin UI.
    """
    daemon = get_daemon()
    db     = get_db()
    ...
    if daemon_running:
        ...
        # Accuracy updater: check for recent errors in log (approximate)
    ...
    color, overall_status = compute_health_color(
        ...
        ollama_reachable         = ollama_status.get("reachable", False),
    )
